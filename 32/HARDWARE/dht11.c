...
static uint8_t dht_read_byte(void)
{
    ...
    for (i = 0; i < 8; i++)
    {
        if (dht_wait(1, 100)) return 0xFF;      /* 等50us低电平结束 */
        delay_us(40);                            /* 40us 后采样: 高=1 低=0 */
        byte <<= 1;
        if (DHT11_READ()) byte |= 1;
        if (dht_wait(0, 100)) return 0xFF;      /* 等本bit高电平结束 */
    }
    return byte;
}
...
uint8_t DHT11_Read(uint8_t *temp_x10, uint8_t *humi_x10)
{
    ...
    if (dht_wait(0, 100)) return 1;     /* DHT11 应答 80us 低 */
    if (dht_wait(1, 100)) return 2;     /* 80us 高 */
    if (dht_wait(0, 100)) return 3;
