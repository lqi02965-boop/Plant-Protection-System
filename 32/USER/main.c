...
/* ---------------- 上报华为云属性(透传MQTT) ---------------- */
static void report_to_cloud(uint8_t temp_x10, uint8_t humi_x10, int pests)
{
    Net_Publish(temp_x10, humi_x10, (pests < 0 ? 0 : pests), g_lamp_on, g_brightness);
}
...
int main(void)
{
    ...
    while (1)
    {
        ...
        {
            last_dht_ms = now;
            if (DHT11_Read(&temp_x10, &humi_x10) == 0)
        ...
        {
            last_report_ms = now;
            report_to_cloud(temp_x10, humi_x10, pest);
        }
    }
}
